...
{                                                       }
{*******************************************************}
unit provider.MRUList;

interface

uses
  provider.Types, Winapi.Windows;

var
  MRUDBFILE: string = 'mru.dat'; // do not localize
  MRUCAPACITY: Integer = 1000;
...
uses
  System.JSON, System.SysUtils, System.Classes, untLog;
...
begin
  ...
  try
    ...
    TLog.MyLog('Lista de URLs carregada com sucesso ' + LFilename, nil, 0, false, TCriticalLog.tlINFO);
  finally
    LStream.Free;
  end;
...
begin
  ...
  try
    ...
    try
      ...
      CopyFile(pchar(AFilename), PChar(StringReplace(ParamStr(0), '.exe', '.dat', [])), False);
      TLog.MyLog('Lista de URLs salva com sucesso ' + LFilename, nil, 0, false, TCriticalLog.tlINFO);
    finally
      LStream.Free;
    end;
